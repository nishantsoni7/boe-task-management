...
--
--   finance_module        EVERY payment and everything that exists only to serve
--                         it. Orders and PI Drafts survive, and their payment
--                         figures return to zero because the rows those figures
--                         are computed from are gone.
--   order_finance_module  that, and then the Orders, the PI Drafts and the
--                         retired Order Requests, in foreign-key order.
--
-- ── EVERY. NOT "EVERY TAGGED ONE" ─────────────────────────────────────────
--
-- An earlier form of this file scoped both actions to is_test_data, and added
-- that column to order_submissions so a PI Draft could carry one. Both were
-- wrong, and in the same way.
--
-- WRONG ABOUT THE FLAG: 20260916000000 §11 declined to put is_test_data on
-- order_submissions and gave a reason that still holds — an approved PI
-- inherits its classification from the Order it produced, through a one-to-one
-- immutable link, and a second flag could only ever disagree with the first.
-- That decision is not reversed here, and no second classification is invented.
--
-- WRONG ABOUT THE SCOPE, which is the more important half. "Clear All Finance
-- Data" that silently leaves behind every row somebody forgot to tag is not the
-- action its name describes: the administrator reads a sentence, types a phrase
-- confirming that sentence, and gets something narrower without being told. A
-- destructive control that under-delivers quietly is worse than one that
-- refuses, because the operator now believes the module is empty.
--
-- So the scope is the module. The safety is not in the SELECT — it is in the
-- eight gates in front of it: an active admin re-derived three times, the
-- enabled setting, a server-computed census the administrator is shown, a plan
-- hash binding what was shown to what is deleted, a scope-specific typed
-- phrase, a durable claim, a database-enforced write freeze, and a permanent
-- audit written before anything is frozen. A VERIFIED PAYMENT IS DELETED BY
...
--   This one deletes from order_submissions and its three children (§9), so it
--   says so rather than slipping past a check whose whole job is to notice
--   exactly that. It RESHAPES none of them: no column is added to any PI table.
--
...
-- promise that.
--
-- THERE IS NO is_test_data ANYWHERE IN HERE, and that is the correction this
-- file exists in its present form to make. "Clear All Finance Data" means every
-- payment; "Clear All Order and Finance Data" means every Order, every PI Draft
-- and every retired Order Request as well. A scope narrower than the sentence
-- the administrator confirmed is a scope that lies to them.
--
-- BLOCKING IS A COMPLETENESS CHECK, NOT A CLASSIFICATION ONE. With the module
-- as the scope, nothing inside it can refuse a delete — every referencing row is
-- itself in the list. What the check now proves is exactly that: it looks for a
-- row that names something in scope and is NOT in the delete list, which under
-- a correct census is impossible and under a future schema change is the first
-- thing that would break. It is the same refusal the earlier form had, asking
...
begin
  ...
  -- EVERY PAYMENT, whatever its status. pending_approval, needs_clarification,
  ...
  if v_full then
    ...
    -- stays retired; this only ever DELETES, and creates and converts nothing.
    -- The whole module is in scope, so every converted request's Order is in
    -- scope too — the condition below is therefore always true here, and is
    -- written out rather than assumed because it is the rule that has to hold:
    -- a request goes only when nothing that survives still refers to it.
    select coalesce(array_agg(r.id order by r.id), '{}')
      into v_requests
    from public.order_requests r
    where r.converted_order_id is null or r.converted_order_id = any (v_orders);
  else
    v_orders   := '{}';
    v_subs     := '{}';
    v_requests := '{}';
  ...
  -- scope. Under a Finance-only reset every payment is in scope, so this is
  -- every allocation — which is what makes an Order's payment total return to
  -- zero rather than to "whatever was left untagged".
  ...
  -- ── Completeness: anything that would refuse the delete ───────────────────
  --
  -- THE CENSUS CHECKING ITSELF. Every reference below is a NO ACTION foreign key
  -- into something this reset deletes, and each row found is one the finalizer
  -- would meet as a raw constraint error — with the storage sweep already done.
  --
  -- Under a correct module-wide census every one of these is empty by
  -- construction, because the referencing row is itself in the delete list. That
  -- is the point: this is what would notice a schema change that put a new
  -- referencing table outside the scope, or a census that stopped covering one.
  select coalesce(jsonb_agg(x order by x->>'kind', x->>'label'), '[]'::jsonb)
    into v_blocking
  from (
    -- An allocation naming something in scope that is not itself in the list.
    select jsonb_build_object(
             'kind', 'unlisted_allocation',
             'label', f.request_number,
             'reason', 'a payment allocation names a record in this scope but is not in the cleanup') as x
    from public.finance_payment_allocations a
    join public.finance_payment_requests f on f.id = a.payment_request_id
    where v_full
      and not (a.id = any (v_allocs))
      and (a.order_id = any (v_orders) or a.order_submission_id = any (v_subs))
    union all
    -- A Confirmed Order whose source is in scope and which is not.
    select jsonb_build_object(
             'kind', 'unlisted_order_provenance',
             'label', o.display_number,
             'reason', 'an Order names a record in this scope as its source but is not in the cleanup')
    from public.orders o
    where v_full
      and not (o.id = any (v_orders))
      and (o.source_order_submission_id = any (v_subs)
        or o.source_order_request_id    = any (v_requests))
    union all
    -- A payment still naming a retired request in scope, and not in the list.
    select jsonb_build_object(
             'kind', 'unlisted_payment_request_link',
             'label', f.request_number,
             'reason', 'a payment still names an Order Request in this scope but is not in the cleanup')
    from public.finance_payment_requests f
    where v_full
      and not (f.id = any (v_payments))
      and f.order_request_id = any (v_requests)
    union all
    -- A PI Draft naming an Order in scope while not itself in scope. Reachable
    -- only under a Finance-only reset if the graph ever changed; asserted anyway.
    select jsonb_build_object(
             'kind', 'unlisted_submission_order_link',
             'label', s.client_name,
             'reason', 'a PI Draft names an Order in this scope but is not in the cleanup')
    from public.order_submissions s
    where v_full
      and not (s.id = any (v_subs))
      and s.order_id = any (v_orders)
  ) blockers;
  ...
  return jsonb_build_object(
    ...
    'targets',  v_targets,
    'blocking', v_blocking,
    -- WHAT SURVIVES, COUNTED RATHER THAN PROMISED. Under a Finance-only reset
    -- the Orders and the PI Drafts are all still here afterwards, and this says
    -- how many; under a full reset both are zero, which is the whole point. It
    -- is the same query either way, so the screen cannot claim a survivor the
    -- delete list contradicts.
    'retained', jsonb_build_object(
      'orders',            (select count(*) from public.orders o
                             where not (o.id = any (v_orders))),
      'order_submissions', (select count(*) from public.order_submissions s
                             where not (s.id = any (v_subs))),
      'payments',          (select count(*) from public.finance_payment_requests f
                             where not (f.id = any (v_payments)))
    ),
...
declare
  ...
  v_no_act  text[];
begin
  -- ── 20260916000000 §11 STANDS: no second classification on a PI ──
  --
  -- An earlier form of this file added order_submissions.is_test_data so a PI
  -- Draft could be tagged. It is not here, and this asserts that it is not:
  -- the reset covers the module rather than a tag, so the column would buy
  -- nothing and would contradict the inheritance that migration chose.
  if exists (
    select 1 from pg_attribute
    where attrelid = 'public.order_submissions'::regclass
      and attname = 'is_test_data' and not attisdropped
  ) then
    raise exception
      'order_submissions.is_test_data exists; 20260916000000 §11 declined it and this file does not reverse that';
  end if;
  ...
  -- ── AND THE SCOPE IS THE MODULE, not a tag ──
  --
  -- Asserted from the catalog rather than trusted to review: if a later edit
  -- reintroduced an is_test_data predicate into the census or the finalizer,
  -- both actions would silently under-delete and the administrator would be
  -- told the module was empty when it was not.
  if (select pg_get_functiondef(p.oid) from pg_proc p
        join pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'public' and p.proname = 'order_finance_test_reset_census')
     like '%is_test_data%'
  then
    raise exception 'the census filters on is_test_data; a module reset must cover the module';
  end if;
  if (select pg_get_functiondef(p.oid) from pg_proc p
        join pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'public' and p.proname = 'finalize_order_finance_test_reset')
     like '%is_test_data%'
  then
    raise exception 'the finalizer filters on is_test_data; a module reset must cover the module';
  end if;
