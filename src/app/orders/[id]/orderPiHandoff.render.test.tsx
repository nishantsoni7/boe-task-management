...
// ── 10. The documents surface has LEFT this page ─────────────────

/**
 * WHAT WAS REMOVED, AND WHAT MUST NOT HAVE BEEN.
 *
 * The Documents section and the Generate Document control were taken off the
 * Confirmed Order screen. The register, its route, its storage rule and its
 * RLS are all untouched and still tested by documentsRoute.test.ts and
 * orderDocuments.test.ts. These pin the removal as a PRESENTATION change, so
 * a later phase reinstating a documents surface starts from an intact service
 * rather than from a half-deleted one.
 */
describe('/orders/[id] no longer draws documents', () => {
  const page = readFileSync(join(process.cwd(), 'src/app/orders/[id]/page.tsx'), 'utf8')

  test('the page neither reads the register nor offers to generate', () => {
    for (const gone of ['order_document_versions', 'ORDER_DOCUMENT_COLUMNS',
                        'buildOrderDocumentsView', 'OrderDocumentsCard',
                        'mayGenerateDocuments', 'ORDER_DOCUMENT_URL_TTL_SECONDS',
                        '/documents`, { method:']) {
      assert.ok(!page.includes(gone), gone + ' is still on the page')
    }
  })

  test('no dead presentation code is left behind in the page-owned components', () => {
    const sections = readFileSync(join(process.cwd(), 'src/app/orders/[id]/OrderPiSections.tsx'), 'utf8')
    assert.ok(!sections.includes('OrderDocumentsCard'))
    assert.ok(!sections.includes('orderDocuments'))
  })
  ...
  test('THE SHARED SERVICE IS UNTOUCHED — only the screen stopped asking', () => {
    for (const kept of ['src/lib/orders/orderDocuments.ts',
      ...
      assert.ok(readFileSync(join(process.cwd(), kept), 'utf8').length > 0, kept)
    }
  })
})
