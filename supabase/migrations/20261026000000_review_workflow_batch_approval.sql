...
begin
  ...
  if c.status <> 'booked' then
    raise exception 'CUSTOMER_REVIEW_TEST_BAD_TRANSITION: A % review cannot be released', c.status
      using errcode = '23514';
  end if;
  ...
  if c.sent_confirmed_at is not null then
    raise exception 'CUSTOMER_REVIEW_TEST_ALREADY_SENT: You confirmed you sent this review, so it can no longer be released'
      using errcode = '23514';
  end if;
  ...
  if v_shots > 0 then
    raise exception 'CUSTOMER_REVIEW_TEST_NOT_READY: Remove the screenshot you attached before releasing this review'
      using errcode = '23514';
  end if;
