...
export type AttributionFixture = {
  ...
  /** What each target must be attributed, by the canonical rule. */
...
export const ATTRIBUTION_FIXTURES: Record<string, AttributionFixture> = {
  A: {
    ...
    directLinkTarget: X,
    allocations: [],
    expected: { [X]: '1000000.00' },
    expectedUnallocated: '0',
    expectedState: 'full',
    note: 'Legacy link with no allocations: the fallback attributes the whole '
        + 'payment, so nothing is free. Reporting it as unallocated would put '
        + 'committed money into the suspense queue.',
  },
  ...
  E: {
    ...
    expected: { [X]: '1000000.00', [Y]: '0' },
    expectedUnallocated: '0',
    expectedState: 'full',
    note: 'A reversed allocation is a withdrawn claim. It does not suppress the '
        + 'direct linkage, so the fallback applies exactly as in A.',
  },
