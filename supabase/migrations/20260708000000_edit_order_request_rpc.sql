-- Order Requests: restore pre-approval editing through a narrowly scoped RPC,
-- and open it to the request's ASSIGNEE alongside the admin.
--
-- Problem this closes
-- -------------------
-- "Edit Request" has been broken for EVERY role, admin included. The page issues
-- a direct `update public.order_requests ... where id = $1`, but this table has
-- had NO update policy for any role since 20260683 §3 and 20260687 §3 dropped
-- order_requests_requester_update / _admin_update to "close the direct-update
-- bypass". PostgREST therefore filters the row out, the statement affects zero
-- rows, and the client reports "This request can no longer be edited." Verified
-- live before writing this migration: an admin's direct UPDATE and the
-- assignee's direct UPDATE each affected 0 rows.
--
-- The fix deliberately does NOT reinstate an UPDATE policy. Table UPDATE stays
-- closed to every client role, exactly as those two migrations intended, and
-- editing joins clarification / rejection / resubmission / reapplication /
-- conversion as one more SECURITY DEFINER RPC with authorization in its body.
--
-- Permission rule implemented here
-- --------------------------------
--     admin  OR  auth.uid() = order_requests.assigned_to
--
-- Editing follows the ASSIGNMENT, not authorship. created_by / requested_by
-- grant nothing on their own (a product decision, confirmed before writing
-- this): a salesperson who raises a request and assigns it to a colleague hands
-- over the edit right with it, and a former assignee loses edit access the
-- instant an admin reassigns. Self-created requests keep working because the
-- creator is normally also the assignee.
--
-- Only an admin may change assigned_to. A non-admin who supplies a different
-- assigned_to is REJECTED rather than silently ignored, so a hand-rolled RPC
-- call cannot quietly reassign a request and cannot be mistaken for success.
--
-- What this migration does NOT do
-- -------------------------------
--   * No UPDATE / INSERT / DELETE policy is created, dropped, or altered.
--   * No status transition. The RPC never writes `status`, so a request cannot
--     be handed back for review through this door — resubmit_order_request and
--     reapply_order_request remain the only paths that change status, and they
--     remain requester-only.
--   * No change to clarification, rejection, conversion, deletion, payment
--     linking (20260707), assignee eligibility, or request numbering.
--   * No new business validation. Amount and client-name checks mirror
--     resubmit_order_request exactly; notably there is still no due_date >=
--     confirm_date rule anywhere in this module, and this migration does not
--     invent one (it would reject existing rows on save).

-- ── 1. New activity event: request_edited ────────────────────────────────────
-- The CHECK is rewritten rather than extended in place (Postgres has no "add
-- value to CHECK"), listing the deployed set verbatim plus the new value.
-- Confirmed against pg_constraint before writing, so nothing is dropped here by
-- omission.
--
-- log_order_request_activity() is intentionally NOT touched: its final branch
-- records nothing when `status` is unchanged, and this RPC never changes
-- status, so the activity row inserted below is the only one an edit produces.
...
-- ── 2. edit_order_request ────────────────────────────────────────────────────
-- Parameter list and order mirror resubmit_order_request / reapply_order_request
-- so the three form modes stay interchangeable on the client. Explicitly typed
-- columns, never a JSON patch: the set of editable columns is fixed here and
...
begin
  ...
  if v_client                is distinct from v_req.client_name         then v_changed := v_changed || 'client_name'; end if;
  if v_assignee              is distinct from v_req.assigned_to         then v_changed := v_changed || 'assigned_to'; end if;
  if p_confirm_date          is distinct from v_req.confirm_date        then v_changed := v_changed || 'confirm_date'; end if;
  if p_due_date              is distinct from v_req.due_date            then v_changed := v_changed || 'due_date'; end if;
  if p_total_value           is distinct from v_req.total_value         then v_changed := v_changed || 'total_value'; end if;
  if p_total_product_value   is distinct from v_req.total_product_value then v_changed := v_changed || 'total_product_value'; end if;
  if p_lead_source           is distinct from v_req.lead_source         then v_changed := v_changed || 'lead_source'; end if;
  if v_notes                 is distinct from v_req.notes               then v_changed := v_changed || 'notes'; end if;
