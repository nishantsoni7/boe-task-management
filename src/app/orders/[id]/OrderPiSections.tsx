...
// ── The PI history ────────────────────────────────────────────────────────────
//
// ONE ORDER, MANY PI VERSIONS (20261113000000). The current approved PI, the
...
// for the people the database will let press them, and the database decides
// again when they do.

const VERSION_TONE: Record<PiVersionTone, { bg: string; color: string; border: string }> = {
  green:   { bg: colors.greenTint, color: colors.green, border: 'rgba(69,168,112,0.3)' },
  amber:   { bg: colors.amberTint, color: colors.amber, border: 'rgba(190,140,40,0.28)' },
  red:     { bg: colors.redTint,   color: colors.red,   border: 'rgba(217,79,79,0.3)' },
  neutral: { bg: colors.raised,    color: colors.secondary, border: colors.border },
}

function PiVersionRow({
  version, heading, opening, onOpen, actions,
}: {
  version: PiVersionView
  heading: string | null
  opening: boolean
  onOpen: ((version: PiVersionView) => void) | null
  actions?: React.ReactNode
}) {
  const tone = VERSION_TONE[version.tone]
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: '13px', fontWeight: 700, color: colors.primary }}>{version.label}</span>
        <span style={{
        ...
        <span style={{ fontSize: '11.5px', color: colors.muted, flex: '1 1 200px', minWidth: 0 }}>
          Uploaded by {version.uploadedBy} · {version.uploadedAt}
          {version.decisionLine ? ` · ${version.decisionLine}` : ''}
        </span>
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginLeft: 'auto' }}>
          {actions}
          {onOpen && version.workbookPath && (
            <button
              type="button"
              className="boe-btn boe-btn-ghost"
              onClick={() => onOpen(version)}
              disabled={opening}
              title={version.workbookName ?? version.label}
            >
              <FileSpreadsheet size={13} strokeWidth={2} />
              {opening ? 'Preparing…' : OPEN_VERSION_LABEL}
            </button>
          )}
        </div>
        {(version.revisionReason || version.decisionReason) && (
          <div style={{ flexBasis: '100%', display: 'flex', flexDirection: 'column', gap: '3px' }}>
            {version.revisionReason && (
              <div style={{ fontSize: '12px', color: colors.secondary, lineHeight: 1.5 }}>
                ...
                <MultilineText>{version.revisionReason}</MultilineText>
              </div>
            )}
            {version.decisionReason && (
              <div style={{ fontSize: '12px', color: colors.red, lineHeight: 1.5 }}>
                <span style={{ color: colors.muted }}>Refused: </span>
                <MultilineText>{version.decisionReason}</MultilineText>
              </div>
            )}
          </div>
        )}
    ...
    <PiCard>
      ...
      <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        {current && (
          <PiVersionRow
            version={current}
            heading={PI_HISTORY_CURRENT_HEADING}
            opening={opening === current.id}
            onOpen={onOpen}
          />
        )}
        {pending && (
