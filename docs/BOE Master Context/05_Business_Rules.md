...
* **The Meeting Inbox is not a table.** An open issue that no meeting has claimed
  IS the Inbox. Nothing is moved out of it and nothing can be lost from it, and
  the source task link cannot be dropped on the way.
...
* **"Earlier" means the same thing it means for an Order** — held before this
  meeting by meeting date, then creation time.
...
* **A meeting viewer never gains task access.** An issue stores a source task
  ID and never a task title. Every task link on a meeting screen is drawn from a
  read made with the VIEWER'S OWN client, so a task that does not come back is
  named as inaccessible rather than linked.
* **Nothing is deleted.** The three tables have no client INSERT, UPDATE, DELETE
  or TRUNCATE privilege and no write policy; every mutation goes through a
  SECURITY DEFINER function that writes the row and its trail entry in ONE
  transaction. There is no Undo on Add to Meeting: no operation removes an
  agenda entry, and Resolve must never stand in for one, because it records that
  the business issue is finished.
* **Visibility.** An issue is readable by whoever can read a meeting it has been
  on, by its creator (so the person who raised it from a task can see what became
  of it), and by a meeting editor or manager (so the Inbox, which has no meeting
  to derive visibility from, can be triaged). Plain `meetings:view` grants
  nothing beyond the meetings that person already sees.
