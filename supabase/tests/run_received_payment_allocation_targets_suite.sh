#!/bin/sh
# Prove 20261216000000 (received_payment_allocation_targets) on a
# DISPOSABLE database:
#
#   1. build the shaped schema (the ledger suite's, plus reserved_order_number
#      and finance_payment_status_is_verified) and install the DEPLOYED
#      bodies of the helpers the RPC and the policies call, extracted from
#      their own migrations;
#   2. run the assertions BEFORE the migration — they must reproduce the defect
#      (a participant's RLS read returns 1 of 4 active allocations) and then stop,
...
#
#   supabase/tests/run_received_payment_allocation_targets_suite.sh <psql host or socket dir> [port]
#
# Creates and drops a database called boe_allocation_targets. Touches nothing
# else and never talks to a linked project. POSIX sh, so it also runs inside a
# stock postgres:alpine container.
set -eu
HOST="${1:?usage: run_received_payment_allocation_targets_suite.sh <psql host or socket dir> [port]}"
PORT="${2:-5432}"
REPO="$(cd "$(dirname "$0")/../.." && pwd)"
DB=boe_allocation_targets
...
$PSQL -d "$DB" -f "$REPO/supabase/tests/_received_payment_allocation_targets_extra_schema.sql" >/dev/null
...
set +e
BEFORE="$($PSQL -d "$DB" -f "$REPO/supabase/tests/received_payment_allocation_targets_assertions.sql" 2>&1)"
set -e
if ! printf '%s' "$BEFORE" | grep -q '0. reproduced'; then
  printf '%s\n' "$BEFORE"; echo "FAIL: the partial-RLS defect did not reproduce"; exit 1
fi
if ! printf '%s' "$BEFORE" | grep -q 'received_payment_allocation_targets.*does not exist'; then
  printf '%s\n' "$BEFORE"; echo "FAIL: expected the suite to stop on the missing RPC"; exit 1
fi
echo "   reproduced: participant reads 1 of 4 active allocations; the RPC does not exist yet"

echo "== applying 20261216000000 (its own apply-time assertions run here)"
$PSQL -d "$DB" -f "$MIG/20261216000000_received_payment_allocation_targets.sql"

echo "== the assertions"
set +e
OUT="$($PSQL -d "$DB" -f "$REPO/supabase/tests/received_payment_allocation_targets_assertions.sql" 2>&1)"
STATUS=$?
set -e
printf '%s\n' "$OUT"
