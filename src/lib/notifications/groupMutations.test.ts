...
describe('34/35. no regression into suppressed territory', () => {
  ...
  test('35. GROUPING added no migration — the one newer file is not its doing', () => {
    ...
    assert.deepEqual(newer, [
      ...
      // Customer Review Outreach, a separate module added later still. Named
      // rather than tolerated by a loosened rule, so a third unexplained file
      // appearing here still fails.
      '20261017000000_customer_review_outreach.sql',
    ], 'only the activity-link column and Customer Review Outreach, both added by later work')
