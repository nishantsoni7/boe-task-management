...
describe('the migration is unapplied, numbered 110, and says its apply order', () => {
  ...
  test('there is no second migration 109 in this tree', () => {
    const files = readdirSync(join(ROOT, 'supabase/migrations'))
    assert.equal(files.filter(f => f.startsWith('20261009')).length, 0)
...
describe('the migration deletes in foreign-key order, and proves the order', () => {
  ...
  test('the ONLY structural change to the PI tables is the classification column', () => {
    // finalApprovalSchema.test.ts and submissionDeletion.test.ts both guard the
    // PI submission tables against outside phases restructuring them, and both
    // now forgive this one statement family. That forgiveness is only safe while
    // this file's structural footprint really is that narrow, so it is asserted
    ...
    assert.deepEqual(structural, [
      'alter table public.order_submissions add column if not exists is_test_data boolean not null default false;',
    ])
    // And no policy on any of those tables is created, altered or dropped.
