 ...
 * as text.
 *
 * Executing it was done against a disposable local stack in production's state
 * (2026-09-26): before it, a signed-in person attaching a payment proof through
 * the app's own attachPaymentProof() failed with "permission denied for
 * function in_test_data_cleanup" (production holds 0 proof rows and 0 proof
 * files for exactly this reason); after it, the proof attaches, the payment
 * stays pending, and a retry with the same idempotency key replays the same
 * payment instead of recording a second one. (#211 showed the same for an
 * Order change request and an Order status change.)
...
describe('focused: the §3 of #211 is not here', () => {
  test('assert_order_amender is not touched by this file', () => {
    assert.doesNotMatch(SQL, /assert_order_amender/, 'its search_path is a separate change, unrelated to proof upload')
  })
})

describe('the writes it exists for are real', () => {
  test('attaching a payment proof is a direct insert into a reset-guarded table', () => {
    const proof = read('src/lib/finance/paymentProof.ts')
    assert.ok(proof.includes("client.from('payment_proof_attachments').insert({"))
    const reset = read('supabase/migrations/20261010000000_order_submission_and_finance_test_data_reset.sql')
    assert.ok(reset.includes("'payment_proof_attachments'"), 'payment_proof_attachments is in the reset guard lock list')
  })

  test('a failed proof never removes the payment, so a retry replays it', () => {
    const proof = read('src/lib/finance/paymentProof.ts')
    assert.doesNotMatch(proof, /from('finance_payment_requests')s*.delete/)
  })
