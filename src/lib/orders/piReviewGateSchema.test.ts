/**
 * 20261113000000 — the PI decision, the attached-payment submission rule, PI
...
const FILE = '20261113000000_order_submission_pi_review_gate_versions_and_production.sql'
...
describe('lineage', () => {
  test('it is the newest migration, and follows the applied ledger', () => {
    ...
    assert.equal(files[files.length - 2], '20261112000000_exclude_partner_from_performance_population.sql')
  })
...
describe('the migration checks itself at apply time', () => {
  test('columns, functions, grants, indexes, the backfill, the seed and the policies', () => {
    ...
    assert.ok(code.includes("raise notice '20261113000000 applied"))
  })
})
