...
create table if not exists public.customer_review_generation_claims (
  ...
  -- How many times this key has been claimed. A retry after a failure is a
  -- second attempt at the same request and is counted rather than hidden, so
  -- "why were we billed twice for one key" has an answer on the row.
...
--              a permanent refusal. It is recorded as an attempt count on the
--              next claim rather than as a tombstone nobody would ever clear.
--
