// Clearing a MODULE of test data: what it means, what it says, and what it is
...
export const RESET_TITLE: Record<ResetScope, string> = {
  finance_module:       'Clear Finance Data',
  order_finance_module: 'Clear All Order & Finance Data',
}

/** What each scope removes, in the words the card shows. */
export const RESET_REMOVES: Record<ResetScope, readonly string[]> = {
  finance_module: [
    'Every test payment and payment request',
    'Every allocation of those payments, to Orders and to PI Drafts',
    'Their proof files, and their activity history',
    'Order and Finance notifications about them',
  ],
  order_finance_module: [
    'Everything Clear Finance Data removes',
    'Every test Confirmed Order, with its activity, amendments and generated documents',
    'Every test PI Draft, with its product lines, images, activity and correction requests',
    'Retired Order Request test records whose Order is in the same cleanup',
...
export const RESET_RETAINS: Record<ResetScope, readonly string[]> = {
  finance_module: [
    ...
    'Any payment that is not test data',
    'Users, permissions, Access Control, payroll, attendance and tasks',
  ],
  order_finance_module: [
    'Any Order, PI Draft or payment that is not test data',
    'Users, permissions, Access Control and module configuration',
    'Payroll, attendance, tasks, assets and showroom data',
    'The cleanup audit — this facility never deletes its own record of what it did',
  ],
}
...
export const RESET_ACKNOWLEDGEMENT =
  'I understand this permanently deletes the records listed above from the connected'
  + ' project, and that it cannot be undone.'
...
const FAILURE_COPY: Record<ResetFailureCode, { message: string; retryable: boolean }> = {
  ...
  FORBIDDEN: {
    message: 'Only an active administrator can clear Order and Finance test data.', retryable: false },
  DISABLED: {
  ...
  REASON_REQUIRED: {
    message: 'Enter why this test data is being removed.', retryable: false },
  CONFIRMATION_INVALID: {
  ...
  BLOCKED: {
    message: 'Records that are not test data would have to be deleted, so nothing was cleared.',
    retryable: false },
  IN_PROGRESS: {
