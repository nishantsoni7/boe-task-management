    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
        ...
        <span style={{ fontSize: '12px', fontWeight: 600, color: colors.primary }}>
          About Cancelled Tasks
        </span>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {[
          'Cancelled tasks are removed from active workflow and do not count toward performance.',
          'You can restore a cancelled task from its detail page to return it to active work.',
        ].map((text, i) => (
...
function CancelledTaskCard({
  task, userId, userMap, onClick, isMobile,
}: {
  task: Task
  userId: string
...
function EmptyState() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px', gap: '6px' }}>
      ...
      <span style={{ fontSize: '13px', fontWeight: 500, color: colors.secondary }}>No cancelled tasks</span>
      <span style={{ fontSize: '12px', color: colors.muted }}>Cancelled tasks will appear here.</span>
    </div>
  )
}

// ─── Page ─────────────────────────────────────────────────────────────────────
function MyCancelledTasksContent() {
  ...
  return (
    <>
      <DashboardLayout profile={profile} title="My Tasks – Cancelled" onSignOut={handleLogout}>
        ...
        <div style={{ marginBottom: '14px' }}>
          ...
          <div style={{ fontSize: '11px', color: colors.muted, marginTop: '2px' }}>
            {allTasks.length} task{allTasks.length !== 1 ? 's' : ''} cancelled
          </div>
        </div>
        ...
        <div style={{
          ...
          {assignerOptions.length > 1 && (
            <select
              ...
              style={{ padding: '4px 10px', minWidth: '130px', background: colors.base, border: `1px solid ${colors.border}`, borderRadius: '6px', outline: 'none', fontSize: '11.5px', color: filterAssignedBy ? colors.primary : colors.muted, cursor: 'pointer' }}
            >
...
export default function MyCancelledTasksPage() {
  return (
    <Suspense fallback={<LoadingScreen />}>
      <MyCancelledTasksContent />
    </Suspense>
  )
}
