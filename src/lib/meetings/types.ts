...
export type MeetingOrder = {
  ...
  order_number: string
  /**
   * upper(btrim(order_number)), generated by the database. The key that makes
   * the same Order in two meetings the same Order. Optional so hand-built rows
   * (tests, optimistic state) need not invent it — see orderNumberKey().
   */
...
export const MEETING_ORDER_COLUMNS = [
  'id', 'meeting_id', 'order_number', 'order_number_key', 'order_type', 'customer_name',
...
/**
 * One image attached to an Order's discussion in a meeting (20261203000000).
 * Append-only: there is no path by which a client changes or removes one.
 */
export type MeetingOrderEvidence = {
  id: string
  meeting_id: string
  meeting_order_id: string
  order_number: string
