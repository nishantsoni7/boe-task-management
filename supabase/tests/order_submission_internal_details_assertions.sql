...
-- ═══ 2b. NO LABEL CHOICE EXISTS ════════════════════════════════════════════
-- A non-zero deduction is always "Discount" on the generated PI; there is no
-- column to store another word and no door to set one.

do $$
begin
  perform pg_temp.check(not exists (select 1 from information_schema.columns
     where table_schema = 'public' and table_name = 'order_submissions' and column_name = 'client_deduction_label'),
    'there is no column for a deduction label choice');
  perform pg_temp.check(to_regproc('public.set_order_submission_deduction_label') is null,
    'there is no door to set one');
  raise notice 'section 2b (no deduction label choice) passed';
end $$;
