...
describe('settings', () => {
  test('the active row is the newest, parsed; a missing table falls back to the defaults and says so', async () => {
    ...
    // THE ROW IN THIS FIXTURE CARRIES NO IMAGE REWARD AND NO MONTHLY SUBMISSION
    // RULES, which is exactly the shape of a settings row written before those
    // columns existed. The built-in defaults fill them in rather than letting
    // the parse fail, because this function's whole promise is that it never
    // throws and never returns null — a screen must not go blank because one
    // column is younger than one row.
    assert.deepEqual(active.settings, {
      review_reward_credits: 150,
      image_review_reward_credits: DEFAULT_BOE_CREDIT_SETTINGS.image_review_reward_credits,
